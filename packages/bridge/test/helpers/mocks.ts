...
export class MockWebSocket {
  ...
  constructor(url: string) {
    ...
    if (MockWebSocket.mode === 'open') {
      setTimeout(() => {
        ...
        if (MockWebSocket.autoAuthenticate) {
  ...
  send(data: string): void {
    ...
    if (MockWebSocket.autoAuthenticate && parsed.type === 'client_hello') {
      this.simulateMessage({
        type: 'auth_accepted',
        grantedScopes: [
          'read',
...
export class MockRemNotePlugin {
  ...
  storage = {
    ...
    setLocal: vi.fn(async (key: string, value: unknown): Promise<void> => {
      this.localStorageStore.set(key, value);
    }),

    getLocal: vi.fn(async <T = unknown>(key: string | undefined): Promise<T | undefined> => {
      if (!key) {
        return undefined;
      }
      return this.localStorageStore.get(key) as T | undefined;
    }),
  };
  ...
  private localStorageStore = new Map<string, unknown>();
...
export function createMockRequest(
  ...
  return {
    ...
    action,
    scope: action === 'replace_children' ? 'destructive' : 'write',
    payload,
  };
}
