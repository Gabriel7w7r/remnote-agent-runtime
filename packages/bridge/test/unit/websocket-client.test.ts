...
describe('WebSocketClient', () => {
  ...
  describe('Authenticated hello message', () => {
    it('should send a protocol-v2 client hello after the server challenge', async () => {
      client.connect();
      await wait(10);
      ...
      const helloMessage = ws.sentMessages.find((msg) => {
        ...
        return parsed.type === 'client_hello';
      });
      expect(helloMessage).toBeDefined();

      const parsed = JSON.parse(helloMessage!);
      expect(parsed.type).toBe('client_hello');
      expect(parsed.protocolVersion).toBe(2);
      expect(parsed.bridgeVersion).toBe('0.5.0');
  ...
  describe('Reconnection logic', () => {
    ...
    it('should enter standby reconnect mode after burst attempts are exhausted', async () => {
      ...
      expect(logs.some((log) => log.message.includes('Standby reconnect'))).toBe(true);
      expect(client.getReconnectMetadata().nextRetryAt).toBeDefined();
    });
