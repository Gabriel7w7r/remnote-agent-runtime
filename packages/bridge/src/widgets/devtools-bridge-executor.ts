...
import { getActionPolicy } from '@remnote-agent/protocol';
...
export function registerDevToolsBridgeExecutor(config: DevToolsExecutorConfig): () => void {
  const listener = async (event: Event): Promise<void> => {
    ...
    const policy = getActionPolicy(detail.action);
    if (!policy) {
      config.onLog?.(`Ignored unclassified devtools action: ${detail.action}`, 'warn');
      return;
    }
