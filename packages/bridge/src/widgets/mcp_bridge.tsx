...
export function AutomationBridgeWidget() {
  ...
  const handlePair = useCallback(() => {
    ...
    if (!/^\d{6}$/.test(code)) {
      void plugin.app.toast('Enter the six-digit pairing code shown by Codex.');
      return;
    }
    const command = createBridgeUiCommand('pair', { code });
    console.log(withScopedLogPrefix('widget', 'Storage command out: pair'));
    void plugin.storage.setSession(BRIDGE_UI_COMMAND_STORAGE_KEY, command);
  }, [pairingCode, plugin]);
  ...
  return (
    <div style={{ padding: '12px', fontFamily: 'system-ui, sans-serif', fontSize: '13px' }}>
      ...
      >
        <h3 style={{ margin: 0, fontSize: '14px', fontWeight: 600 }}>RemNote Agent</h3>
        <div
          style={{
            display: 'flex',
            alignItems: 'center',
      ...
      {snapshot.pairingRequired && (
        ...
        >
          <div style={{ fontWeight: 700, color: '#92400e', marginBottom: '5px' }}>
            Pair this RemNote installation
          </div>
          <div style={{ fontSize: '11px', color: '#78350f', marginBottom: '9px' }}>
            Ask Codex for RemNote pairing status, then enter the six-digit code it shows. This is
            required only once.
          </div>
      ...
      {snapshot.authenticated && (
        ...
        >
          Securely paired with {snapshot.grantedScopes.length} scopes and{' '}
          {snapshot.capabilities.filter((capability) => capability.available).length} available
          capabilities.
        </div>
      )}

      {snapshot.authRejectionReason && (
        <div
          style={{
            marginBottom: '12px',
            padding: '9px 10px',
        ...
        >
          Authentication failed: {snapshot.authRejectionReason}
        </div>
      )}
