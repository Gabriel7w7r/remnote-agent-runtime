...
const AgentOperationSchema = z.object({ operation: z.string().min(1) }).catchall(z.unknown());

const EDITOR_READ_OPERATIONS = [
  'get_focused_text',
  'get_selection',
  'get_selected_rem',
  'get_selected_text',
...
const EDITOR_DELETE_OPERATIONS = ['cut', 'delete_selection', 'delete_characters'] as const;
const WINDOW_READ_OPERATIONS = [
  'get_tree',
...
export const EDITOR_TOOL = createAgentTool(
  'remnote_editor',
  ...
  [...EDITOR_READ_OPERATIONS, ...EDITOR_WRITE_OPERATIONS, ...EDITOR_DELETE_OPERATIONS]
);
...
export const QUEUE_TOOL = createAgentTool(
  ...
  [...QUEUE_READ_OPERATIONS, ...QUEUE_WRITE_OPERATIONS]
);
export const CARD_TOOL = createAgentTool(
  'remnote_card',
  'Read card scheduling metadata, append a repetition score, or permanently remove a card.',
  [...CARD_READ_OPERATIONS, 'update_repetition', 'remove']
);
...
export const DAILY_DOCUMENT_TOOL = createAgentTool(
  'remnote_daily_document',
  "Resolve or create today's Daily Document or the Daily Document for an ISO date.",
  ['today', 'date']
);
export const READER_TOOL = createAgentTool(
  'remnote_reader',
  'Create a highlight from the current selection in the active RemNote reader.',
  ['add_highlight']
);
export const AUDIT_LOG_TOOL = {
...
function operationAction(
  operation: string,
  groups: Array<{ operations: readonly string[]; action: string }>
): string {
  const group = groups.find((candidate) => candidate.operations.includes(operation));
  if (!group) throw new Error(`Unsupported operation: ${operation}`);
  return group.action;
}
...
export const PAIRING_STATUS_TOOL = {
  ...
  outputSchema: {
    ...
    properties: {
      ...
      pairingPending: { type: 'boolean' },
      pairingCode: { type: 'string' },
      pairingExpiresAt: { type: 'string' },
      pairedInstallationId: { type: 'string' },
      grantedScopes: { type: 'array', items: { type: 'string' } },
    },
    required: ['paired', 'authenticated', 'pairingPending', 'grantedScopes'],
  },
};

export const RESET_PAIRING_TOOL = {
...
export const CAPABILITIES_TOOL = {
  ...
  outputSchema: {
    ...
    required: ['authenticated', 'grantedScopes', 'capabilities'],
  },
};
...
export function registerAllTools(
  ...
  async function buildStatusResult(): Promise<Record<string, unknown>> {
    ...
    const securityStatus =
      typeof wsServer.getSecurityStatus === 'function' ? wsServer.getSecurityStatus() : {};

    if (!connected) {
      return {
        connected: false,
        serverVersion,
        message: 'RemNote Agent Bridge is not authenticated',
        ...securityStatus,
      };
    }
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      switch (toolName) {
        ...
        case 'remnote_pairing_status': {
          result = wsServer.getSecurityStatus();
          break;
        }

        case 'remnote_reset_pairing': {
          wsServer.resetPairing();
          result = {
            reset: true,
            message:
              'Pairing was reset. Reconnect the RemNote Agent Bridge to generate a new code.',
          };
          break;
        }

        case 'remnote_capabilities': {
          result = {
            authenticated: wsServer.isConnected(),
            grantedScopes: wsServer.getGrantedScopes(),
            capabilities: wsServer.getCapabilities(),
          };
          break;
        }
        ...
        case 'remnote_editor': {
          ...
          const action = operationAction(args.operation, [
            { operations: EDITOR_READ_OPERATIONS, action: 'editor_read' },
            { operations: EDITOR_WRITE_OPERATIONS, action: 'editor_write' },
            { operations: EDITOR_DELETE_OPERATIONS, action: 'editor_delete' },
          ]);
        ...
        case 'remnote_window': {
          const args = AgentOperationSchema.parse(request.params.arguments);
          const action = operationAction(args.operation, [
            { operations: WINDOW_READ_OPERATIONS, action: 'window_read' },
            { operations: WINDOW_WRITE_OPERATIONS, action: 'window_write' },
          ]);
          result = await wsServer.sendRequest(action, args);
          break;
        }
        ...
        case 'remnote_queue': {
          const args = AgentOperationSchema.parse(request.params.arguments);
          const action = operationAction(args.operation, [
            { operations: QUEUE_READ_OPERATIONS, action: 'queue_read' },
            { operations: QUEUE_WRITE_OPERATIONS, action: 'queue_write' },
          ]);
          result = await wsServer.sendRequest(action, args);
          break;
        }
        ...
        case 'remnote_card': {
          const args = AgentOperationSchema.parse(request.params.arguments);
          const action = operationAction(args.operation, [
            { operations: CARD_READ_OPERATIONS, action: 'card_read' },
            { operations: ['update_repetition'], action: 'card_write' },
            { operations: ['remove'], action: 'card_delete' },
          ]);
          result = await wsServer.sendRequest(action, args);
          break;
        }
        ...
        case 'remnote_rem': {
          const args = AgentOperationSchema.parse(request.params.arguments);
          const action = operationAction(args.operation, [
            { operations: REM_READ_OPERATIONS, action: 'rem_read' },
            { operations: REM_WRITE_OPERATIONS, action: 'rem_write' },
        ...
        case 'remnote_knowledge_base': {
          const args = AgentOperationSchema.parse(request.params.arguments);
          operationAction(args.operation, [
            { operations: ['current', 'is_primary'], action: 'kb_read' },
          ]);
          result = await wsServer.sendRequest('kb_read', args);
          break;
        }

        case 'remnote_daily_document': {
          const args = AgentOperationSchema.parse(request.params.arguments);
          operationAction(args.operation, [
            { operations: ['today', 'date'], action: 'daily_document_write' },
          ]);
          result = await wsServer.sendRequest('daily_document_write', args);
          break;
        }
        ...
        case 'remnote_reader': {
          const args = AgentOperationSchema.parse(request.params.arguments);
          operationAction(args.operation, [
            { operations: ['add_highlight'], action: 'reader_write' },
          ]);
          result = await wsServer.sendRequest('reader_write', args);
          break;
        }

        case 'remnote_audit_log': {
          const args = z
            .object({ limit: z.number().int().min(1).max(1000).default(100) })
            .parse(request.params.arguments);
          result = { entries: wsServer.getAuditEntries(args.limit) };
          break;
        }
