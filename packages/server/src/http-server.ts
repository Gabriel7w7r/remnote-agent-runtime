...
export class HttpMcpServer {
  ...
  private setupRoutes(): void {
    this.app.get('/health', (_req, res) => {
      res.json({ status: 'ok', service: this.serverInfo.name, version: this.serverInfo.version });
    });

    // The local MCP endpoint is privileged. Clients receive the token through the
    // runtime bootstrap, never through an unauthenticated network endpoint.
    this.app.use('/mcp', (req, res, next) => {
      const header = req.headers.authorization;
      const token = header?.startsWith('Bearer ')
        ? header.slice('Bearer '.length).trim()
        : undefined;
      if (!this.authStore.verifyHttpToken(token)) {
        res.setHeader('WWW-Authenticate', 'Bearer realm="remnote-agent"');
        res.status(401).json({
          jsonrpc: '2.0',
          error: { code: -32001, message: 'Missing or invalid RemNote Agent bearer token' },
          id: null,
        });
        return;
      }
