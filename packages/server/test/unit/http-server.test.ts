...
vi.mock('../../src/websocket-server.js', () => ({
  WebSocketServer: class MockWebSocketServer {
    ...
    getSecurityStatus = vi.fn().mockReturnValue({ authenticated: true });
    getGrantedScopes = vi.fn().mockReturnValue(['read', 'write']);
...
describe('HttpMcpServer', () => {
  ...
  const authenticatedFetch = (input: string | URL, init: RequestInit = {}): Promise<Response> => {
    const headers = new Headers(init.headers);
    headers.set('Authorization', `Bearer ${authStore.getHttpToken()}`);
  ...
  beforeEach(async () => {
    ...
    authStore = new AuthStore(mkdtempSync(join(tmpdir(), 'remnote-agent-http-test-')));
  ...
  describe('Server Lifecycle', () => {
    it('should start and bind to specified port', async () => {
      ...
      const response = await authenticatedFetch(`http://127.0.0.1:${port}/mcp`, {
        method: 'POST',
        headers: {
    ...
    it('should stop and unbind from port', async () => {
      ...
      await expect(
        authenticatedFetch(`http://127.0.0.1:${port}/mcp`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Accept: 'application/json, text/event-stream',
  ...
  describe('Session Requests', () => {
    it('should handle requests with valid session ID', async () => {
      ...
      // Initialize session
      const initResponse = await authenticatedFetch(`http://127.0.0.1:${port}/mcp`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Accept: 'application/json, text/event-stream',
  ...
  describe('Session Termination', () => {
    it('should terminate session via DELETE request', async () => {
      ...
      // Terminate session
      const deleteResponse = await authenticatedFetch(`http://127.0.0.1:${port}/mcp`, {
        method: 'DELETE',
        headers: {
          Accept: 'application/json, text/event-stream',
          'mcp-session-id': sessionId,
  ...
  describe('Bearer Authentication', () => {
    it('rejects an unauthenticated MCP request', async () => {
      ...
      const response = await globalThis.fetch(`http://127.0.0.1:${port}/mcp`, {
        method: 'POST',
    ...
    it('keeps the health endpoint unprivileged and token-free', async () => {
      await httpServer.start();
      await waitForHttpServer(port);

      const response = await globalThis.fetch(`http://127.0.0.1:${port}/health`);
      expect(response.status).toBe(200);
      expect(await response.json()).toMatchObject({ status: 'ok', service: 'test-server' });
    });
  });
  ...
  describe('Logging', () => {
    ...
    it('should log new session initialization', async () => {
      ...
      await authenticatedFetch(`http://127.0.0.1:${port}/mcp`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Accept: 'application/json, text/event-stream',
