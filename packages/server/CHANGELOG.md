...
- Add secure one-time bridge pairing, nonce-bound HMAC reconnect authentication, local bearer authentication, and
  capability-scope enforcement.
- Add editor, pane, review queue, card, Rem, knowledge-base, Daily Document, reader, and privacy-preserving audit tools.
- Add SDK coverage inventory and lock validation across all 276 public methods in the pinned RemNote plugin SDK.
- Add reproducible runtime and RemNote bridge release archives with SHA-256 checksums and a release manifest.

### Changed

- Rebrand the local integration as RemNote Agent and synchronize the protocol, server, and bridge at version `0.19.0`.
- Reject non-loopback server configuration and classify actions by capability scope and risk level.
- Require the MCPB and stdio proxies to load the locally stored HTTP token before forwarding requests.

### Security

- Replace the historical auto-approved OAuth compatibility path with enforced local bearer authentication.
- Store audit metadata and payload hashes without note contents or credentials.
